...
export async function updateSessionTitle(
  ...
  const sessionKey = String(session.session_id ?? (splitKey(key).chatId || key));
  return {
    key: sessionKey,
    ...splitKey(sessionKey),
...
export async function listSkills(
  ...
  const body = await request<{ skills: Array<Partial<SkillSummary> & { name?: string; description?: string; tags?: string[] }> }>(
