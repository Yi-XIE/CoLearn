  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (boot.token) saveSecret(boot.token);
        const token = boot.token ?? "";
        setState((current) => ({
          ...current,
          token: token || current.token,
          client: new ColearnWsClient({
            url: deriveWsUrl(boot.ws_path, token),
            baseUrl: "",
          }),
        ...
        console.error("Failed to bootstrap webui", error);
        if (cancelled) return;
...
function Shell({
  ...
  const onRequestRename = useCallback(async (key: string, currentLabel: string) => {
    const nextTitle = window.prompt(
      t("chat.renamePrompt", { defaultValue: "Rename this chat", title: currentLabel }),
      currentLabel,
    );
