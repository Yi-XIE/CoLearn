...
const SETTINGS_TITLE = "Workspace Settings";
    ...
    <div className="relative flex h-full w-full overflow-hidden">
      ...
      <main className="relative flex h-full min-w-0 flex-1 flex-col">
        ...
        {view === "knowledge" && (
          <PanelView
            title="知识花园"
            subtitle="资料库、检索结果与文档 grounding 会在这里汇集。"
            onToggleSidebar={toggleSidebar}
            theme={theme}
            onToggleTheme={toggle}
            hideSidebarToggleOnDesktop={desktopSidebarOpen}
          >
            <InfoCard
              title="资料来源"
              body="这里会成为 CoLearn 文档资料、索引状态与检索入口的主要空间。"
            />
            <InfoCard
              title="LightRAG"
              body="LightRAG 已经接入 runtime_v2。下一轮会把资料导入、索引刷新和基于来源的检索能力放到这里。"
            />
          </PanelView>
        )}
        {view === "memory" && (
          ...
          >
            <InfoCard
              title="记忆状态"
              body="Memory 已经作为一等工具接入 runtime_v2。这里会逐步展示学习连续性、阻塞点和回忆轨迹。"
            />
            <InfoCard
              title="这里会放什么"
              body="近期记忆更新、重要保留事实、连续性提示，以及后续的 Dream 式整理能力都会放在这里。"
            />
          </PanelView>
        )}
        {view === "skills" && (
          ...
          >
            <InfoCard
              title="技能空间"
              body="这里会承载精选技能、可复用提示词，以及支持 CoLearn 学习会话的动作卡片。"
            />
            <InfoCard
              title="接下来会放什么"
              body="下一轮会把内置技能、带资料感知的助手，以及面向目标的工具组合逐步放进来。"
            />
          </PanelView>
        )}
        {view === "settings" && (
