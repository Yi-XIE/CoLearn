    ...
    def __init__(
        ...
        *,
        ...
        self._allowed_modules = (
            set(allowed_modules) if allowed_modules is not None else set(_DEFAULT_ALLOWED_MODULES)
        )
    ...
    def discover(self) -> list[type[Tool]]:
        ...
        for _importer, module_name, _ispkg in pkgutil.iter_modules(self._package.__path__):
            if module_name.startswith("_") or module_name in _SKIP_MODULES:
                continue
            if self._allowed_modules and module_name not in self._allowed_modules:
                continue
            try:
