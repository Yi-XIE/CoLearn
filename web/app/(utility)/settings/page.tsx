...
function SettingsPageContent() {
  ...
  const [catalogEditable, setCatalogEditable] = useState(true);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const catalogResponse = await apiFetch(apiUrl("/api/v1/settings/catalog"));
        if (!catalogResponse.ok) {
          throw new Error(`Catalog fetch failed: ${catalogResponse.status}`);
        }
        const catalogPayload =
          (await catalogResponse.json()) as CatalogPayload;
        setCatalog(catalogPayload.catalog);
        setDraft(cloneCatalog(catalogPayload.catalog));
        setCatalogEditable(true);
      } catch (err) {
      ...
      try {
        const providersResponse = await apiFetch(
          apiUrl("/api/v1/settings/providers"),
        );
        ...
        const providersPayload =
          (await providersResponse.json()) as ProvidersPayload;
        setProviders(providersPayload.providers);
      } catch (err) {
        console.error("Failed to load provider choices:", err);
      }
                        ...
                        <div className="sm:col-span-2">
                          ...
                          {activeService === "embedding" && (
                            <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                              {t(
                                "Embedding requests are sent to this URL exactly; CoLearn does not append /embeddings or /api/embed at request time.",
                              )}
                            </p>
                          )}
                        </div>
