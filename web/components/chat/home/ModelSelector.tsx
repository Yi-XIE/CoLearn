...
export default function ModelSelector({
  ...
  const contextWindowLabel = selectedOption
    ? formatContextWindow(selectedOption.context_window)
    : "";
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-expanded={open}
        className={`inline-flex h-[28px] max-w-[186px] shrink-0 items-center gap-1.5 rounded-full px-2 text-[11px] font-medium transition-colors sm:max-w-[240px] ${
          disabled
            ? "cursor-not-allowed text-[var(--border)]"
            : open
              ? "bg-[var(--primary)]/[0.04] text-[var(--foreground)]"
      ...
      >
        ...
        {contextWindowLabel ? (
          <span className="shrink-0 text-[10px] text-[var(--muted-foreground)]/70">
            {contextWindowLabel}
          </span>
        ) : null}
        <ChevronDown
          size={10}
          className={`shrink-0 text-[var(--muted-foreground)]/70 transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
