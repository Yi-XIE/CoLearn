...
import type { MemoryReferenceFile } from "@/lib/learning-context";

interface MemoryPickerProps {
  open: boolean;
  initialFiles: MemoryReferenceFile[];
  onClose: () => void;
  onApply: (files: MemoryReferenceFile[]) => void;
}

interface MemoryOption {
  key: MemoryReferenceFile;
  label: string;
  description: string;
  icon: LucideIcon;
}
...
const MEMORY_OPTIONS: MemoryOption[] = [
  ...
  },
  {
    key: "mastery",
    label: "Mastery",
    description: "Inject the current mastery projection and tracked misconceptions.",
    icon: Brain,
  },
  {
    key: "event_store",
    label: "Event Store",
    description: "Inject the recent learning event stream for continuity.",
    icon: ScrollText,
  },
];
...
export default function MemoryPicker({
  ...
  const [selected, setSelected] = useState<MemoryReferenceFile[]>(initialFiles);
  ...
  const toggle = (key: MemoryReferenceFile) => {
    setSelected((prev) =>
