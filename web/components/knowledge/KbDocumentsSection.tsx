...
export default function KbDocumentsSection({
  ...
  const blockedReason = !uploadable
    ? needsReindex
      ? t(
          "This source library is in a legacy index format and needs re-index before upload.",
        )
      : status !== "ready"
        ? t(
            "This source library is currently {{status}} and cannot accept uploads yet.",
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-[11.5px] text-[var(--muted-foreground)]">
          {t(
            "Drop files here to add them to this source library. New files are indexed against the active embedding model.",
          )}
        </p>
      </div>
